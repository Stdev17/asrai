...
| file | what it holds |
|---|---|
| `test_asrai.py` | vocabulary search/get/locales, lint rules, `measure` determinism, record validation and layer rules, `doctor` lock and drift, the MCP tools in-process **and** over stdio, refusal of malformed input at both trust boundaries, every number the documents claim against `claims.json`, the scanner that looks for the ones no row claims, the MCP tool surface against its token budget, and every index in the repository against what it indexes — a directory without a page, a table that has stopped listing a file, a public name a table advertises and no module defines |
