...
Eleven modules include transports, core owners and version metadata; **nothing in the core imports a
...
`run` is drawn and owns one: it is the only module that opens the asset.
...
| `run.py` | one run: the asset read once, its subjects, and the identity a form is stamped with | `open_run`, `ledger`, `SUBJECTS_MAX` | every family in a run sees the same pixels, the same subjects and the same `run_sha256`. A family is handed a run and cannot open a file, choose a subject or name a run, so two of them cannot disagree about the thing they are both looking at |
| `light.py` | the surface pass: `surfaces.v1` instantiated as a two-phase ledger over a run | `pass_`, `surfaces`, `profiles`, `sentences`, `for_reader` | direction may be measured; magnitude may not be invented. Every number it returns is a measurement, a count or a sign |
