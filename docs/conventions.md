...
| `sentences(result)` | `explain`, `summarise`, `report`; a `profile=` parameter on `ledger` | the name says what comes back, and the signature says what may happen: it takes a finished result and returns strings, so the reader it serves can never be the reason a verdict reads the way it does. The rejected parameter is the one that matters — threading an audience into `ledger` is the cheap way to add a profile and the one way to let expression outrank judgment, which [the 2026-09-17 review](review/2026-09-17-profile-alignment.md) settles the other way. `explain` and `report` both promise a document; this returns one line per finding |
| `for_reader(result, profile)` | `rendered`, `as_read`, `with_sentences` | `rendered` was rejected on a wrong prior: in a tool about game art it reads as producing pixels, which is a stated non-goal, and a function that returns prose must not borrow that word. The chosen name says the dimension — which reader — and it exists so `None` means one thing in both transports: no profile is the default, and a result is complete before any is applied |
