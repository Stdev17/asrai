...
| [`2026-09-20-shape-and-the-gate-consumers.md`](2026-09-20-shape-and-the-gate-consumers.md) | the package author, choosing the second family | that a family supplies the eight gates rather than occupying a layer, and that `shape` is next because it supplies three of them from four categories and needs nothing new. Supersedes the document above it on three points: the family is not called readability, because `perception` has twenty-three terms and no measurable one; `reading` owns nothing, because a projection owns nothing, which puts the realm back at seven nodes and the §7 forecast back as written; and the thresholds come from a pairwise corpus of an artist's references rather than from asking one for a cutoff on a standard deviation. Counts what `light` costs: zero of eight gates, twenty-five terms across seven categories |
